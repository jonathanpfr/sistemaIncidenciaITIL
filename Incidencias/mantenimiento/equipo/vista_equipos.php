        ...
        <div   class='centrar'>
            <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                    <?php
                    $vent = new equipo();
                    $consulta = $vent->get_todos_equipo();
                    for ($i = 0; $i < sizeof($consulta); $i++) {
                        $id_tra = $consulta[$i]["id_equipos"];
                        $id_area = $consulta[$i]["id_area"];
                        $nombre = $consulta[$i]["nom_equipos"];
                        echo "<tr>";
                        echo "<td>";
                        echo "<a href='modulos/modificar/modificar1.php?id_equipos=$id_tra&id_area=$id_area&nombre_equipo=$nombre'>Modificar</a>";
                        echo "</td>";
                        echo "<td>";
                        echo $consulta[$i]["id_equipos"];
                        echo "</td>";
                        echo "<td>";
                        echo $consulta[$i]["nom_equipos"];
                        echo "</td>";
                        echo "<td>";
                        echo $consulta[$i]["nombre_area"];
                        echo "</td>";

                        echo "<td>";

                        echo "<a onclick='return confirm(\"Esta Seguro de Eliminar al Equipo : $nombre ?\")' href='modulos/eliminar/eliminar.php?id_equipos=$id_tra'>Eliminar</a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                    ?>
