        ...
        <div   class='centrar'>
            <table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tbody>
                    <?php
                    $vent = new usuario();
                    $consulta = $vent->get_usuario();
                    for ($i = 0; $i < sizeof($consulta); $i++) {
                        $id_usuario = $consulta[$i]["id_usuario"];
                        $nombre = $consulta[$i]["razon_social"];
                        $telefono = $consulta[$i]["telefono"];
                        $direccion = $consulta[$i]["direccion"];
                        $usuario = $consulta[$i]["usuario"];
                        $password = $consulta[$i]["password"];
                        echo "<tr>";
                        echo "<td>";
                        echo "<a href='modulos/modificar/modificar1.php?id_usuario=$id_usuario&nombre_usu=$nombre&telefono=$telefono&direccion=$direccion&usuario=$usuario&password=$password'>Modificar</a>";
                        echo "</td>";
                        echo "<td>";
                        ...
                        echo "<a onclick='return confirm(\"Esta Seguro de Eliminar al area : $nombre ?\")' href='modulos/eliminar/eliminar.php?id_usuario=$id_usuario'>Eliminar</a>";
                        echo "</td>";
                        echo "</tr>";
                    }
                    ?>
